#!/bin/bash
LAMBDANAME="brevity-program-get"
...
aws lambda create-function --function-name $LAMBDANAME --runtime python3.8 --handler lambda_function.lambda_handler --role arn:aws:iam::000017942944:role/brevity-lambda --code S3Bucket=brevity-deploy,S3Key=infra/$LAMBDANAME.zip --description 'Retrieves program information.' --timeout 15 --package-type Zip