...
def prepareManual(programName,inputBucketName):
    
    scriptStatus = generateScriptManual(programName,inputBucketName)
    return scriptStatus

def generateScriptManual(programName, inputBucketName):
...
sh $HOME/security/run/{programName}/stepfunctions-{programName}.sh"""
    ...
    # Upload file to S3
    object_name = 'manual-' + programName + '.sh'
...
def generateInstallScriptManual(inputBucketName):
...
export AWS_DEFAULT_REGION=us-east-1"""
   ...
   # Load AWS access keys for s3 synchronization
    ...
    # Upload file to S3
    object_name = 'bounty-startup-manual.sh'
