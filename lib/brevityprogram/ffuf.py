...
import io, json, shutil, ast
import brevitycore.core
import urllib.request

def prepareFfuf(programName,inputBucketName):

    awsRegionsStatus = generateAWSRegionsList(inputBucketName)
    #scriptStatus = generateScriptFfuf(programName,inputBucketName, inputPath, outputPath)
    return awsRegionsStatus

def generateAWSRegionsList(inputBucketName):
    filePath = 'https://raw.githubusercontent.com/RhinoSecurityLabs/pacu/master/pacu/modules/service_regions.json'
    fileBuffer = io.StringIO(newline='\n')
    with urllib.request.urlopen(filePath) as url:
        data = json.loads(url.read().decode())
    for item in data["all"]:
        fileBuffer.write(item + '\n')
    #with open('awsregions.txt', 'w') as fd:
    #    fileBuffer.seek(0)
    #    shutil.copyfileobj(fileBuffer, fd)
    # Upload file to S3
    object_name = 'awsregions.txt'
    object_path = 'wordlists/' + object_name
...
def generateScriptFfuf(programName, inputBucketName, inputPath, outputPath):
